...
def _reset_run_state() -> None:
    ...
    a previous run's expanders and charts sit underneath whatever is now selected and
    read as that selection's output -- a real misdiagnosis this shipped with: a
...
# Distinguishes "no selection observed yet" from any real selection, so the first
# render adopts what it finds instead of clearing results a caller just handed the
# session (which is exactly how a run result is supplied in tests, and how one
# survives the rerun that follows a run finishing).
_UNSET = object()


def _sync_selection(selection: tuple) -> None:
    """Clear the displayed run whenever the selection behind it changes.

    ``selection`` is everything that determines what a run would be OF: the config
    source, the config directory, and the target TLR-* directory. One check covers
    every way of changing it -- switching source, generating a new config set,
    picking another collection or another TLR-* directory -- so the results pane
    always matches the current selection or is empty.
    """
    previous = st.session_state.last_selection
    st.session_state.last_selection = selection
    if previous is not _UNSET and previous != selection:
        _reset_run_state()
...
def _init_session_state():
    defaults = {
        ...
        # the first render adopts what it finds rather than clearing results a caller
        # had just handed the session.
        "last_selection": _UNSET,
...
def main():
    ...
    # Must run before the results pane below, and for every source: it is what keeps
    # a previous run's results from being read as this selection's output.
    _sync_selection((config_source, config_dir, target_risk_dir))
